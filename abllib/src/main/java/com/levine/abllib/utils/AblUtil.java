...
public class AblUtil {
    ...
    public static void addSuspensionWindowView(Context context, View view) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (Settings.canDrawOverlays(context)) {
                addSuspensionView(context, view, -1, -1);
            } else {
     ...
     * @param view
     */
    public static void addSuspensionView(Context context, View view, int width, int height) {

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (!Settings.canDrawOverlays(context)) {
                ToastUtils.showShort("请先开启允许显示在其他应用上权限");
                openDrawOverlaysAnth(context);
                return;
            }
        }
        ...
        layoutParams.width = width;//窗口的宽和高
